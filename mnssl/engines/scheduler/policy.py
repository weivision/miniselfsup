...
class SwAV(BaseScheduler):
    ...
    def __init__(self, cfg=None, optimizer=None):
        ...
        warmup_lr_schedule = np.linspace(cfg.start_warmup, cfg.base_lr, cfg.iter_per_epoch * cfg.warmup_epochs)
        iters = np.arange(cfg.iter_per_epoch * (cfg.epochs - cfg.warmup_epochs))
        cosine_lr_schedule = np.array([cfg.final_lr + 0.5 * (cfg.base_lr - cfg.final_lr) * (1 + \
                            math.cos(math.pi * t / (cfg.iter_per_epoch * (cfg.epochs - cfg.warmup_epochs)))) for t in iters])
