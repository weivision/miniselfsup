...
class BYOL(BaseMethod):
    ...
    def __init__(self, cfg):
        ...
        for param_o, param_t in zip(self.backbone.parameters(),
                                    self.backbone_t.parameters()):
            param_t.data.copy_(param_o.data)
            param_t.requires_grad = False

        for param_o, param_t in zip(self.neck.parameters(),
                                    self.neck_t.parameters()):
            param_t.data.copy_(param_o.data)
            param_t.requires_grad = False
    ...
    def forward(self, inputs):
        ...
        loss = 2. * (self.head(online_z1, target_z2)['loss'] + \
                     self.head(online_z2, target_z1)['loss'])
    ...
    def iter_update(self):
        ...
        if (self.iteration + 1) % self.update_interval == 0:
            self.momentum = self.end_momentum - (self.end_momentum - self.base_momentum) * \
                            (cos(pi * self.iteration / float(self.max_iter)) + 1) / 2
